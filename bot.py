...
class Bot(metaclass=ABCMeta):
    ...
    @staticmethod
    def click_on(img):
        x = pag.locateCenterOnScreen(
            img, confidence=config.CLICK_ON_MATCHING_CONFIDENCE)
        # log the click location
        logging.debug('Clicked on {}'.format(x))
        if x is None:
            pag.screenshot(
                '{}/debug_{}.png'.format(config.LOGS_DIR, time.time()))
            logging.error('Cannot find {} on screen'.format(img))
            raise ValueError('Cannot find {} on screen'.format(img))
        # divide the x and y by 2 to get the center of the image
        pag.click((x[0] / 2, x[1] / 2))
    ...
    def _wait_for_game_completion(self):
        ...
        self.wait_for(config.BUTTON_GAME_NEXT, do_all_checks=False)
        self.click_on(config.BUTTON_GAME_NEXT)
    ...
    def _is_present(self, img):
        self._wait_location = pag.locateCenterOnScreen(
            img, confidence=config.WAIT_FOR_MATCHING_CONFIDENCE)
    ...
    def _check_selected_hero(self, hero_images, hero_name):
        for img in hero_images:
            if self._is_present(img):
                ...
                return
        logging.error(
            'Incorrect hero expected. Hero required is {}, but was not found'.format(hero_name))
        raise ValueError(
            'Incorrect hero expected. Hero required is {}, but was not found'.format(hero_name))
    ...
    def main(self, required_hero=None):
        ...
        while True:
            ...
            try:
                ...
                logging.warning(
                    'FailSafe detected, stopping game {}.'.format(self._game_counter))
                exit(1)
